...
import { SwipeAISettings, PriorityLevel, CATEGORY_CONFIG } from "@/lib/domains/swipe-settings.domain";
...
export default function JobSwipePage() {
  ...
  const [swipeSettings, setSwipeSettings] = useState<SwipeAISettings | null>(null);
  ...
  const detectJobCategory = useCallback((job: Job): { mainCategory: string; jobTitle?: string } => {
    try {
      ...
      const skillsArray = Array.isArray(job.required_skills) ? job.required_skills : [];
      const skillsLower = skillsArray.map(s => String(s || '').toLowerCase()).join(' ');
  ...
  const applyAISettingsToJobs = useCallback((jobs: Job[], settings: SwipeAISettings | null): Job[] => {
    ...
    const jobsWithScores = jobs.map((job) => {
      try {
        ...
        if (jobTitle && settings.jobTitlePriorities?.[jobTitle]) {
          priorityScore = getPriorityScore(settings.jobTitlePriorities[jobTitle]);
        } else {
          // Otherwise, use the main category priority
          const mainCategoryPriority = settings.mainCategoryPriorities?.[mainCategory];
  ...
  const fetchJobsAndInitSwipes = useCallback(async (overrideSettings?: SwipeAISettings | null) => {
    if (!user) return;
    ...
    try {
      ...
      const [jobsResponse, likedResponse, dislikedResponse, savedJobsResponse, settingsResponse] = await Promise.all(
        [
          ...
          overrideSettings ? Promise.resolve({ success: true, settings: overrideSettings }) : getSwipeAISettings(user.uid),
        ]
      );
      ...
      const settingsToUse = overrideSettings || (settingsResponse.success ? settingsResponse.settings : null) || swipeSettings;
  ...
  }, [user, profile, applyAISettingsToJobs]); // Removed swipeSettings to prevent infinite loop
          ...
          <div className="flex flex-col items-center justify-center">
            ...
            >
              {isFinished ? (
                ...
                >
                  <Card className="w-full h-full overflow-hidden border border-blue-100 shadow-md">
                    ...
                    <CardFooter className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-background to-transparent pt-16 pb-4">
                      <div className={`flex items-center w-full ${profile?.role === "GUEST" ? "justify-around" : "justify-between"}`}>
                        <Button
                          variant="outline"
                          size="lg"
                          className="rounded-full h-14 w-14 bg-white dark:bg-gray-800 border-blue-200 hover:bg-blue-50"
            ...
            <div className="flex justify-end">
              <SwipeSettingsPanel
                ...
                onSettingsChange={async (newSettings) => {
                  console.log("Settings changed, updating and refreshing jobs:", newSettings);
