...
import { withSubscriptionAction } from "../middleware/subscription-utils";
import {
  InterviewSetup,
  InterviewResponses,
  InterviewQuestion,
...
const generateInterviewQuestionsBase = async (
...
const evaluateInterviewResponsesBase = async (
  data: InterviewResponses
): Promise<InterviewFeedback> => {
  try {
    const { name, position, experience, questions, responses } =
      interviewResponsesSchema.parse(data);
    ...
    if (jsonMatch) {
      ...
      return {
        ...
        detailedFeedback: questions.map((question) => ({
          question,
          feedback:
            "Response was adequate but could be improved with more specific examples.",
        })),
      };
    }
  } catch (error) {
...
export const generateInterviewQuestions = withSubscriptionAction(
  generateInterviewQuestionsBase
);
export const evaluateInterviewResponses = withSubscriptionAction(
  evaluateInterviewResponsesBase
);
