...
async function uploadResumeFile(
    userId: string,
    file: File
): Promise<{ url: string; filePath: string; success: boolean; error?: string }> {
    try {
        ...
        return { url: '', filePath: '', success: false, error: "Failed to upload file" };
    }
}
 ...
 */
export async function getResumeById(resumeId: string): Promise<{ resume: Resume | null; error?: string }> {
    try {
        const db = getDb();
    ...
    try {
        ...
        if (data.file) {
            ...
            if (file) {
                ...
                if (oldFilePath) {
                    try {
                        await getStorage().bucket(process.env.NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET).file(oldFilePath).delete();
                    } catch (e) {
 ...
 * Delete a resume
 */
export async function deleteResume(resumeId: string, userId: string): Promise<ResumeResponse> {
    try {
        const db = getDb();
        const resumeRef = db.collection('Resumes').doc(resumeId);
        const doc = await resumeRef.get();
        ...
        if (filePath) {
            try {
                await getStorage().bucket(process.env.NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET).file(filePath).delete();
            } catch (e) {
