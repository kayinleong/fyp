...
export async function saveJob(jobId: string): Promise<SavedJobResponse> {
    try {
        ...
        const existingQuery = db.collection(savedJobsCollection)
...
export async function unsaveJob(jobId: string): Promise<SavedJobResponse> {
    try {
        ...
        const savedJobQuery = db.collection(savedJobsCollection)
            .where('user_id', '==', userId)
            .where('job_id', '==', jobId)
            .limit(1);
 ...
 */
export async function getSavedJobs(userId?: string): Promise<SavedJobsResponse> {
    try {
        ...
        const savedJobsQuery = db.collection(savedJobsCollection)
 ...
 */
export async function isJobSaved(jobId: string): Promise<{ isSaved: boolean; error?: string }> {
    try {
        const sessionResponse = await validateSession();
        const userId = sessionResponse.user?.uid;
 ...
 */
export async function getSavedJobsWithDetails(userId?: string): Promise<{ jobs: any[]; error?: string }> {
    try {
